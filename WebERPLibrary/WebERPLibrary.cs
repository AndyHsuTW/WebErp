...
namespace WebERPLibrary
{
    ...
    public class D_EmpId
    {
        ...
        public static List<D_EmpIdData> GetDEmpId(D_EmpIdfilterOption FilterOption, string StartRow)
        {
            var List = new List<D_EmpIdData>();
            using (var conn = new SqlConnection(MyConnStringList.AzureGoodeasy))
            {
                conn.Open();
...
b.cnf0702_bname,c.cnf1003_char01 as DName,d.cnf1003_char01 as TName,a.* 
from taf10 a
left join cnf07 b on a.taf1008_bcode=b.cnf0701_bcode
left join cnf10 c on a.taf1009_dcode=c.cnf1002_fileorder and c.cnf1001_filetype='P01'
left join cnf10 d on a.taf1010_jcode=c.cnf1002_fileorder and c.cnf1001_filetype='P02'
where a.status <>'D'  
                ";
                ...
                if (!String.IsNullOrEmpty(FilterOption.BCode))
                {
                    GetFilterStr("taf1008_bcode", FilterOption.BCode, cmd);
                }
                if (!String.IsNullOrEmpty(FilterOption.EmpCnName))
                {
                    GetFilterStr("taf1004_cname", FilterOption.EmpCnName, cmd);
                }
                if (!String.IsNullOrEmpty(FilterOption.EmpId))
                {
                    GetFilterStr("taf1001_empid", FilterOption.EmpId, cmd);
                }
                if (!String.IsNullOrEmpty(FilterOption.FirstName))
                {
                    GetFilterStr("taf1002_firstname", FilterOption.FirstName, cmd);
                }
                if (!String.IsNullOrEmpty(FilterOption.LastName))
                {
                    GetFilterStr("taf1003_lastname", FilterOption.LastName, cmd);
                }
                if (!String.IsNullOrEmpty(FilterOption.Keyword))
                {
                    GetKeyWordStr(FilterOption.Keyword, cmd);
    ...
    public class D_EmpIdfilterOption
    {
        public string BCode { get; set; }
        public string EmpId { get; set; }
        public string EmpCnName { get; set; }
