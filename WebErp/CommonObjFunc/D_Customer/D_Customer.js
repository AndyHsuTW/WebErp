...
(function () {
    ...
    function onLoaded(bootstrap, FunctionButton, uibPagination, loadingHelper, UserLog) {
        ...
        Vue.component('d_customer_component',
            {
                template: '\
                        <div>\
                            <ul class="app-title">\<li>客戶基本資料查詢v18.06.06</li></ul>\
                            <div class="app-body">\
                                    ...
                                    <br><div class="filter-div">\
                                        <table class="">\
                                            ...
                                            <tr>\
                                                ...
                                                <td><input type="text" v-model="Filter.Keyword">&nbsp;&nbsp;<span style="font-weight:bold">筆數 : {{D_CustomerList.length}}</span>\
                                                </td>\
                                            </tr>\
                                        </table>\
                                    </div>\
                                        ...
                                        <div class="scroll-table">\
                                            <table class="table table-bordered ">\
                                                ...
                                                <tbody  id="table-content" v-bind:style={width:tbodywidth}>\
                                                    <tr class="rowclass" v-for="D_CustomerData in D_CustomerList" >\
                                                        <td class="" v-if="typeof callback == \'function\'">\
                                                        <button type="button" role="button" class="btn btn-default" @click="callback(D_CustomerData)">送出</button>\
                                                        </td>\
                                                        <td class="col-xs-1">{{D_CustomerData.BCode}}</td>\
                                                        <td class="col-xs-1">{{D_CustomerData.EmpId}}</td>\
                                                        <td class="col-xs-1">{{D_CustomerData.EmpCnName}}</td>\
                                                        <td class="col-xs-1">{{D_CustomerData.Mobile}}</td>\
                                                        <td class="col-xs-1">{{D_CustomerData.Phone}}</td>\
                                                        <td class="col-xs-1">{{D_CustomerData.ZipCode}}</td>\
                                                        <td class="col-xs-1">{{D_CustomerData.CAddress}}</td>\
                                                        <td class="col-xs-1">{{D_CustomerData.Email}}</td>\
                                                        <td class="col-xs-1">{{D_CustomerData.OnBoardDate}}</td>\
                                                        <td class="col-xs-1">{{D_CustomerData.Dept}}</td>\
                                                        <td class="col-xs-1">{{D_CustomerData.Title}}</td>\
                                                        <td class="col-xs-1"><img v-bind:src="D_CustomerData.Photo" v-if="D_CustomerData.Photo!=\'\'"></td>\
                ...
                methods: {
                    ...
                    SeachAjax: function () {
                        ...
                        this.ajaxXhr = $.ajax({
                            type: 'POST',
                            url: rootUrl + "CommonObjFunc/D_Customer/Ajax/D_CustomerHandler.ashx",
                            cache: false,
                            data: {
                            ...
                            success: function (result) {
                                ...
                                for (var i in list) {
                                    vueObj.D_CustomerList.push(list[i]);
                                }
